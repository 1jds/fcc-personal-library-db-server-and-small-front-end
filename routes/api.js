...
module.exports = function(app, myDatabase) {
  ...
  app.route('/api/books')
    .get(async function(req, res) {
      ...
      const responseArr = allBooksInDB.map(({ _id, title, comments } = item) => {
        return {
          _id,
          title,
  ...
  app.route('/api/books/:id')
    .get(async function(req, res) {
      ...
      try {
        ...
        console.error(error);
        res.send("Error: the id entered was invalid and could not be converted to a new ObjectId")
        return
      }
      const specificBookRequested = await myDatabase.findOne({ _id: bookIdToSearch })
