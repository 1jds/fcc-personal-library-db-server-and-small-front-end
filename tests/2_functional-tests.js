          ...
          chai
            ...
            .end(function (err, res) {
              ...
              assert.equal(
                res.body.title,
                "test book title",
                "book title should be the string test book title"
              );
              done();
            });
        });
      ...
      test("Test GET /api/books/[id] with valid id in db", function (done) {
        chai
          ...
          .end(function (err, res) {
            ...
            assert.exists(
              res.body.comments,
              "db documents should contain a comments property"
            );
            done();
          });
      });
        ...
        test("Test POST /api/books/[id] with comment", function (done) {
          chai
            ...
            .end(function (err, res) {
              ...
              assert.exists(
                res.body.comments,
                "testing comments array so needs to exist"
              );
              assert.exists(res.body._id, "needs to have an id");
              assert.equal(
                res.body.title,
                "test book title",
                "db document needs a title"
              );
              done();
            });
        });
          ...
          chai
            ...
            .end(function (err, res) {
              ...
              assert.equal(
                ...
                "correct response should be the string missing required field comment"
              );
              done();
            });
        });
        ...
        chai
          ...
          .end(function (err, res) {
            ...
            assert.equal(
              res.text,
              "delete successful",
              "response message indicates that the delete was successful"
            );
          });
        done();
      });
        ...
        chai
          ...
          .end(function (err, res) {
            ...
            assert.equal(
              res.type,
              "text/html",
              "delete attempt no book exists"
            );
            assert.equal(
              res.text,
              "no book exists",
              "response message indicates that the delete was unsuccessful because no such book exists"
            );
          });
        done();
      });
